    ...
    <div className="relative shrink-0 w-full">
      <div className="content-stretch flex flex-col gap-[20px] items-start px-[32px] py-[28px] relative w-full">
        <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0">
          ...
          <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#737373] text-[12px]">
            <p className="leading-none">User initiated runs stay at the top level. Auto generated runs are grouped under each parent run.</p>
          </div>
        </div>
        <Frame20 />
        <Frame19 />
...
type RunStatus = "complete" | "running" | "failed" | "queued";
type RunInitiator = "user" | "auto";
...
type RunHierarchy = {
  topLevelRuns: RecentRun[];
  childrenByParent: Record<string, RecentRun[]>;
};

const AUTO_RUN_PREVIEW_COUNT = 5;
...
const churnAutoRunStatuses: RunStatus[] = [
  "complete",
  "complete",
  "complete",
  "complete",
...
const churnAutoRunNames = [
  "Data quality scan",
  "Feature importance pass",
  "Model baseline candidate",
  "Hyperparameter sweep 1",
  "Hyperparameter sweep 2",
  "Sensitivity checks",
  "Cohort segmentation",
  "Anomaly backfill",
  "Outlier explanation",
...
const churnAutoRuns: RecentRun[] = churnAutoRunStatuses.map((status, index) => ({
  id: `run-001-auto-${index + 1}`,
  name: churnAutoRunNames[index],
  agent: "Researcher",
  createdAt: new Date(Date.parse("2026-03-05T08:32:00.000Z") + index * 2 * 60 * 1000).toISOString(),
  status,
  initiator: "auto",
  parentId: "run-001",
}));

const memoAutoRuns: RecentRun[] = [
  {
    id: "run-003-auto-1",
    name: "Financial appendix extraction",
    agent: "Researcher",
    createdAt: "2026-03-05T10:24:00.000Z",
    status: "complete",
    initiator: "auto",
    parentId: "run-003",
  },
  {
    id: "run-003-auto-2",
    name: "Slide narrative rewrite",
    agent: "Researcher",
    createdAt: "2026-03-05T10:26:00.000Z",
    status: "running",
    initiator: "auto",
    parentId: "run-003",
  },
  {
    id: "run-003-auto-3",
    name: "Risk statement reconciliation",
    agent: "Researcher",
    createdAt: "2026-03-05T10:28:00.000Z",
    status: "queued",
    initiator: "auto",
    parentId: "run-003",
  },
...
const RECENT_RUNS: RecentRun[] = [
  {
    id: "run-003",
    name: "Quarterly board memo synthesis",
    agent: "Researcher",
...
> = {
  complete: {
    label: "Complete",
    containerClass: "bg-[#173b29] border-[#173b29]",
    dotClass: "bg-[#4ade80]",
    textClass: "text-[#4ade80]",
  },
  running: {
    label: "In progress",
    containerClass: "bg-[#1f1f1f] border-[#1f1f1f]",
    dotClass: "animate-pulse bg-[#a3a3a3]",
    textClass: "text-[#a3a3a3]",
  },
  failed: {
    label: "Failed",
    containerClass: "bg-[#3f1d1d] border-[#3f1d1d]",
    dotClass: "bg-[#f87171]",
    textClass: "text-[#fca5a5]",
  },
  queued: {
    label: "Queued",
    containerClass: "bg-[#1f2937] border-[#1f2937]",
    dotClass: "bg-[#93c5fd]",
    textClass: "text-[#bfdbfe]",
  },
};

function sortRunsByCreatedAtDesc(a: RecentRun, b: RecentRun) {
  return Date.parse(b.createdAt) - Date.parse(a.createdAt);
}

function buildRunHierarchy(runs: RecentRun[]): RunHierarchy {
  const topLevelRuns: RecentRun[] = [];
  const childrenByParent: Record<string, RecentRun[]> = {};

  for (const run of runs) {
    if (run.parentId) {
      if (!childrenByParent[run.parentId]) {
        childrenByParent[run.parentId] = [];
      }

      childrenByParent[run.parentId].push(run);
      continue;
    }
  ...
  topLevelRuns.sort(sortRunsByCreatedAtDesc);

  for (const childRuns of Object.values(childrenByParent)) {
    childRuns.sort(sortRunsByCreatedAtDesc);
  }
...
const RECENT_RUN_HIERARCHY = buildRunHierarchy(RECENT_RUNS);

function summarizeStatuses(runs: RecentRun[]) {
  ...
  const statusOrder: RunStatus[] = ["running", "queued", "failed", "complete"];

  return statusOrder
    .filter((status) => counts[status] > 0)
    .map((status) => `${counts[status]} ${RUN_STATUS_META[status].label.toLowerCase()}`)
    .join(" | ");
}
...
function RecentRunStatusPill({ status }: { status: RunStatus }) {
  const style = RUN_STATUS_META[status];

  return (
    <div className={`${style.containerClass} border content-stretch flex gap-[8px] h-[28px] items-center justify-center overflow-hidden px-[8px] relative rounded-[10px] shrink-0`}>
      <div aria-hidden className={`${style.dotClass} rounded-[9999px] shrink-0 size-[8px]`} />
      <div className={`flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[14px] text-ellipsis whitespace-nowrap ${style.textClass}`} style={{ fontVariationSettings: "'wdth' 100" }}>
        <p className="leading-[20px] overflow-hidden">{style.label}</p>
      </div>
    </div>
  );
}

function RecentRunActionsButton() {
  return (
    <button className="content-stretch flex items-center p-[8px] relative shrink-0 transition-colors hover:bg-[#171717]" type="button">
      <div className="flex flex-col font-['SF_Pro:Medium',sans-serif] font-medium justify-center leading-[0] overflow-hidden relative shrink-0 text-[#737373] text-[14px] whitespace-nowrap">
...
function RecentRunAgentBadge({
  ...
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0 w-[192px]">
      <div className="bg-[#1f1f1f] border border-[#1f1f1f] content-stretch flex gap-[8px] h-[26px] items-center justify-center max-w-[192px] overflow-hidden px-[8px] relative rounded-[10px] shrink-0">
        <div aria-hidden className="bg-[#a3a3a3] rounded-[9999px] shrink-0 size-[8px]" />
        <div className="flex flex-[1_0_0] flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] min-h-px min-w-px overflow-hidden relative text-[#a3a3a3] text-[14px] text-ellipsis whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
          <p className="leading-[16px] overflow-hidden">{agent}</p>
        </div>
      </div>
      <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] mt-[4px] overflow-hidden relative shrink-0 text-[#737373] text-[12px] text-ellipsis whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        <p className="leading-none overflow-hidden">{initiator === "user" ? "User initiated" : "Auto generated"}</p>
      </div>
    </div>
  );
}
...
function RecentRunRow({
  ...
  const secondaryText = isChild
    ? "Auto generated branch run"
    : hasChildren
      ? `${childCount} auto runs${childStatusSummary ? ` | ${childStatusSummary}` : ""}`
      : "No auto runs";

  return (
    <div className="relative shrink-0 w-full">
      <div aria-hidden className="absolute border-[#171717] border-b border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center px-[20px] py-[12px] relative w-full">
          <div className="content-stretch flex flex-[1_0_0] gap-[14px] items-center min-h-px min-w-px relative">
            <Checkbox1 />
            <div className="flex flex-[1_0_0] items-start min-h-px min-w-px relative">
              <div className="content-stretch flex items-center justify-center relative shrink-0 w-[20px]">
                {!isChild && hasChildren ? (
                  <button
                    aria-expanded={isExpanded}
                    className="bg-[#1f1f1f] border border-[#262626] content-stretch flex h-[16px] items-center justify-center overflow-hidden rounded-[4px] shrink-0 text-[#a3a3a3] text-[12px] w-[16px] hover:bg-[#262626]"
                  ...
                  <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[#525252] text-[12px] whitespace-nowrap">
                    <p className="leading-none">{isChild ? "->" : ""}</p>
                  </div>
                )}
              </div>
              <div className="content-stretch flex flex-[1_0_0] items-start justify-between min-h-px min-w-px pl-[8px] pr-[10px] relative">
                <div className="content-stretch flex flex-col items-start min-h-px min-w-px relative">
                  <div className="flex flex-col font-['SF_Pro:Medium',sans-serif] font-medium justify-center leading-[0] overflow-hidden relative shrink-0 text-[#f5f5f5] text-[14px] text-ellipsis whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
                    <p className="leading-none overflow-hidden">{run.name}</p>
                  </div>
                  <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] mt-[4px] overflow-hidden relative shrink-0 text-[#737373] text-[12px] text-ellipsis whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
                    <p className="leading-none overflow-hidden">{secondaryText}</p>
                  </div>
                </div>
                {!isChild && hasChildren ? (
                  <div className="bg-[#1f1f1f] border border-[#262626] content-stretch flex h-[22px] items-center justify-center overflow-hidden px-[8px] relative rounded-[9999px] shrink-0">
                    <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[#a3a3a3] text-[12px] whitespace-nowrap">
                      <p className="leading-none">{`${childCount} auto`}</p>
                    </div>
                  </div>
                ) : null}
              </div>
          ...
          <RecentRunAgentBadge agent={run.agent} initiator={run.initiator} />
          <div className="content-stretch flex items-center relative shrink-0 w-[140px]">
            <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[#a3a3a3] text-[14px] text-ellipsis whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
              <p className="leading-[20px] overflow-hidden">{formatCreatedAt(run.createdAt)}</p>
            </div>
          </div>
          <div className="content-stretch flex flex-col items-start relative shrink-0 w-[190px]">
            <RecentRunStatusPill status={run.status} />
          </div>
          <RecentRunActionsButton />
        </div>
      </div>
    </div>
  );
    ...
    <div className="relative shrink-0 w-full">
      ...
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center px-[20px] py-[10px] relative w-full">
          <div className="content-stretch flex flex-[1_0_0] gap-[14px] items-center min-h-px min-w-px relative">
            <div className="bg-[#0a0a0a] relative rounded-[4px] shrink-0 size-[16px]">
              <div aria-hidden className="absolute border border-[#404040] border-solid inset-0 pointer-events-none rounded-[4px]" />
            </div>
            <button
              className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[#a3a3a3] text-[13px] text-left whitespace-nowrap hover:text-[#f5f5f5]"
              onClick={onShowMore}
              type="button"
            >
              <p className="leading-none">{`Show ${hiddenCount} more auto runs`}</p>
            </button>
          </div>
...
function RecentsTable() {
  const [expandedParents, setExpandedParents] = useState<Record<string, boolean>>(() => {
    ...
    for (const topLevelRun of RECENT_RUN_HIERARCHY.topLevelRuns) {
      if ((RECENT_RUN_HIERARCHY.childrenByParent[topLevelRun.id] ?? []).length > 0) {
        initialExpandedParents[topLevelRun.id] = false;
      }
    }

    return initialExpandedParents;
  });
  const [visibleAutoRunCountByParent, setVisibleAutoRunCountByParent] = useState<Record<string, number>>({});

  const totalAutoRuns = Object.values(RECENT_RUN_HIERARCHY.childrenByParent).reduce((count, childRuns) => count + childRuns.length, 0);
  ...
  const handleShowMoreAutoRuns = (parentId: string, totalChildren: number) => {
    setVisibleAutoRunCountByParent((current) => ({
      ...current,
      [parentId]: Math.min((current[parentId] ?? AUTO_RUN_PREVIEW_COUNT) + AUTO_RUN_PREVIEW_COUNT, totalChildren),
    }));
  };
      ...
      <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[inherit] w-full">
        <div className="relative shrink-0 w-full">
          <div className="content-stretch flex items-center justify-between px-[20px] py-[10px] relative w-full">
            <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[#737373] text-[12px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
              <p className="leading-none">{`${RECENT_RUN_HIERARCHY.topLevelRuns.length} user initiated runs`}</p>
            </div>
            <div className="flex flex-col font-['SF_Pro:Regular',sans-serif] font-normal justify-center leading-[0] overflow-hidden relative shrink-0 text-[#737373] text-[12px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
              <p className="leading-none">{`${totalAutoRuns} auto generated runs grouped by parent`}</p>
            </div>
          </div>
          <div aria-hidden className="absolute border-[#171717] border-b border-solid inset-0 pointer-events-none" />
        </div>
        <Frame50 />
        {RECENT_RUN_HIERARCHY.topLevelRuns.map((parentRun) => {
          const childRuns = RECENT_RUN_HIERARCHY.childrenByParent[parentRun.id] ?? [];
          const childStatusSummary = summarizeStatuses(childRuns);
          const isExpanded = expandedParents[parentRun.id] ?? false;
          const previewCount = visibleAutoRunCountByParent[parentRun.id] ?? AUTO_RUN_PREVIEW_COUNT;
          const visibleChildRuns = isExpanded ? childRuns.slice(0, previewCount) : [];
          const hiddenChildCount = isExpanded ? childRuns.length - visibleChildRuns.length : 0;

          return (
            <div className="w-full" key={parentRun.id}>
              <RecentRunRow
                childCount={childRuns.length}
                childStatusSummary={childStatusSummary}
                isExpanded={isExpanded}
                onToggleChildren={childRuns.length > 0 ? () => handleToggleParent(parentRun.id) : undefined}
                run={parentRun}
              />
              {visibleChildRuns.map((autoRun) => (
                <RecentRunRow isChild key={autoRun.id} run={autoRun} />
              ))}
              {hiddenChildCount > 0 ? <ShowMoreAutoRunsRow hiddenCount={hiddenChildCount} onShowMore={() => handleShowMoreAutoRuns(parentRun.id, childRuns.length)} /> : null}
            </div>
          );
        })}
      </div>
      <div aria-hidden className="absolute border border-[#171717] border-solid inset-0 pointer-events-none rounded-[12px]" />
    </div>
  );
}
